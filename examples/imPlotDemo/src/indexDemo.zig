...
const plots_histogram= @import("plots/Histogram.zig");
...
// Config
const config_config = @import("config/Config.zig");
// Help
const help_help = @import("help/Help.zig");
...
pub fn imPlotDemoTabs() !void {
    if (ig.igBeginTabBar("ImPlotDemoTabs", 0)) {
        ...
        if (ig.igBeginTabItem(c.ICON_FA_CHART_LINE ++ " Plots", null, 0)) {
            ...
            try demoHeader("Histogram", plots_histogram.demo_Histogram);
        ...
        if (ig.igBeginTabItem("Config", null, 0)) {
            defer ig.igEndTabItem();
            try config_config.demo_Config();
        }
        if (ig.igBeginTabItem("Help", null, 0)) {
          defer ig.igEndTabItem();
          try help_help.demo_Help();
        }
    }
}
